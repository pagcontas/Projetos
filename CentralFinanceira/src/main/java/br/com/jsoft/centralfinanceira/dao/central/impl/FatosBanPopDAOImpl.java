 ...
 */
@Stateless
public class FatosBanPopDAOImpl extends BaseDAOImpl<FatosBanPop> implements FatosBanPopDAO {

    @Override
    public void updateUnitarioConvenio(Long id, BigDecimal valor, Integer periodo) {
        String queryString = " UPDATE " + FatosBanPop.class.getName()
                + " SET unitarioconvenio = ?1";
        Query query = null;
        queryString += " WHERE conveniobanpop.id = ?2 and periodo = ?3";
    ...
    public List<ComissaoConvenioVO> listBoletoConvenio() {
        ...
        String queryString = "select periodo, conveniobanpop_id, c.nome, "
                + "sum(qtd) qtd, sum(arrecadado) arrecadado, avg(unitarioconvenio) unitarioconvenio, "
                + "sum(qtd*unitarioloja) comissaoloja from financeira.fatosbanpop f "
                + "inner join cadastro.conveniobanpop c on (c.id=f.conveniobanpop_id) "
                + "group by conveniobanpop_id, c.nome, periodo "
                + "order by conveniobanpop_id, periodo";
    ...
    public List<ComissaoLojaVO> listBoletoLoja() {
        ...
        String queryString = "select periodo, loja_id, l.nome, "
                ...
                + "sum(qtd*unitarioconvenio) comissaoconvenio from financeira.fatosbanpop f "
