 ...
 */
@Stateless
public class FatosbbDAOImpl extends BaseDAOImpl<Fatosbb> implements FatosbbDAO {
    @Override
    public void updateUnitarioConvenio(Long id, BigDecimal valor, Integer periodo) {
        String queryString = " UPDATE " + Fatosbb.class.getName()
                + " SET unitarioconvenio = ?1";
        Query query = null;
        queryString += " WHERE conveniobb.id = ?2 and periodo = ?3";
    ...
    public List<ComissaoConvenioVO> listBoletoConvenio() {
        ...
        String queryString = "select periodo, conveniobb_id, c.nome, "
                + "sum(qtd) qtd, sum(arrecadado) arrecadado, avg(unitarioconvenio) unitarioconvenio, "
                + "sum(qtd*unitarioloja) comissaoloja from financeira.fatosbb f "
                + "inner join cadastro.conveniobb c on (c.id=f.conveniobb_id) "
                + "group by conveniobb_id, c.nome, periodo "
                + "order by conveniobb_id, periodo";
    ...
    public List<ComissaoLojaVO> listBoletoLoja() {
        ...
        String queryString = "select periodo, loja_id, l.nome, "
                ...
                + "sum(qtd*unitarioconvenio) comissaoconvenio from financeira.fatosbb f "
